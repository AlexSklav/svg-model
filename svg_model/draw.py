...
def draw_shapes_svg_layer(df_shapes: pd.DataFrame, shape_i_columns: Union[str, List[str]],
                          layer_name: str, layer_number: int = 1,
                          use_svg_path: bool = True) -> StringIO:
    """
    Draw shapes as a layer in an SVG file.
    ...
    Returns:
        StringIO: A file-like object containing SVG XML source.
    ...
    svg_root = dwg.g(id=f'layer{layer_number}',
    ...
    for shape_i, df_shape_i in df_shapes.groupby(shape_i_columns):
        ...
        if not use_svg_path:
            ...
            commands = [f'M {x},{y}' for x, y in vertices]
            commands.extend([f'L {x},{y}' for x, y in vertices[1:]])
            while vertices[0] == vertices[-1]:
                # Start is equal to the end of the path, but we will use the `'Z'`
...
def draw_lines_svg_layer(df_endpoints: pd.DataFrame, layer_name: str, layer_number: int = 1) -> StringIO:
    """
    Draw lines defined by endpoint coordinates as a layer in an SVG file.
    ...
    line_layer = dwg.g(id=f'layer{layer_number}',
                       **{'inkscape:label': layer_name,'inkscape:groupmode': 'layer'})

    for i, (x1, y1, x2, y2) in df_endpoints[coord_columns].iterrows():
        line_i = dwg.line(start=(x1, y1), end=(x2, y2), id=f'line{i}',
